...
export VERSION=$(curl -s https://api.github.com/repos/chaitin/HummerRisk/releases/latest | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
...
function get_installer() {
  ...
  if [ ! -d "hummerrisk-installer-${VERSION}" ]; then
    curl -LOk -m 60 -o "${hummerrisk_online_file_name}" https://github.com/chaitin/hummerrisk/releases/download/"${VERSION}"/"${hummerrisk_online_file_name}" || {
